...
/**
 * TitlesManager
 * - ユーザーの合計話数・合計登録数を取得して獲得済み称号を判定
 * - 獲得済み称号のみ選択可能にして、選択すると profiles.current_title を更新
 * - トースト通知を表示
 *
 * 使い方（例）:
 *   <TitlesManager />
 *
 * ※ 既に supabase を別モジュールで初期化している場合はそちらを import してください。
 */
...
export default function TitlesManager() {
  ...
  const [earned, setEarned] = useState([]); // earned title ids
  ...
  const evaluateEarned = (p) => {
    ...
    const earnedIds = TITLE_DEFINITIONS.filter((t) => {
      ...
      if (t.type === "both") return chapters >= t.threshold.chapters && registered >= t.threshold.registered;
      return false;
    }).map((t) => t.id);
  ...
  if (!user) {
    return <div className="p-4 text-red-500">ログインしてください</div>;
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4 text-sm text-gray-600">
        あなたの合計話数: <strong>{profile?.total_chapters ?? 0}</strong> / 合計登録数: <strong>{profile?.total_registered ?? 0}</strong>
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center justify-between gap-3 p-3 rounded-lg border transition ${
                isEarned ? "bg-white hover:scale-105" : "bg-gray-50 opacity-60 cursor-not-allowed"
      ...
      {/* シンプルなスタイル / アニメーション: 必要なら globals.css に移動 */}
      <style jsx>{`
        .animate-fadeInUp {
